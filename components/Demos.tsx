      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {demos.map((demo, index) => (
            ...
            >
              {/* Grande image - Responsive height */}
              ...
              {/* Contenu minimal superposé - Responsive padding */}
              <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-6 md:p-8 z-20">
                {/* Tag de résultat */}
                <div className="inline-block mb-2 sm:mb-3 md:mb-4">
                  <span className="bg-[#2D5A27] text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-full text-[10px] sm:text-xs font-bold uppercase tracking-wider">
                    {demo.id === 'restaurant' ? '+42% CONVERSION' : 'AUTOMATION'}
                  </span>
                </div>
                
                {/* Nom du projet */}
                <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-1 sm:mb-2 group-hover:text-[#2D5A27] transition-colors drop-shadow-lg">
                  {demo.title}
                </h3>
